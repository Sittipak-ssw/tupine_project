	...
	**           MANDATORY SCRIPT
	...
	public function course()
	{
		...
		if (isset($user_idtoken)) {
			...
			$this->load->view('course', compact('users'));
		} else {
			redirect('');
			die();
		}
	...
	public function user()
	{
		...
		// exit();
		if (isset($user_idtoken)) {
			$this->db->select('users.id as users_id,users.email as email,users.password as password,users.firstnamethai as firstnamethai,users.lastnamethai as lastnamethai');
			$this->db->where('id', $user_idtoken);
			$users = $this->db->get('users')->row();
			$this->load->view('user', compact('users'));
		} else {
			redirect('');
			die();
		}
	...
	public function enroll_show()
	{
		...
		if (isset($user_idtoken)) {
			...
			$this->db->where('id', $user_idtoken);
			$users = $this->db->get('users')->row();
			$this->load->view('enroll_show', compact('users'));
		} else {
			redirect('');
			die();
		}
	...
	public function ipen()
	{
		...
		if (isset($user_idtoken)) {
			...
			$this->db->where('id', $user_idtoken);
			$users = $this->db->get('users')->row();
			$this->load->view('ipen', compact('users'));
		} else {
			redirect('');
			die();
		}
